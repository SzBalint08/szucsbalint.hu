...
function spin() {
  ...
  if (points < bet) {
    resultText.textContent = "Insufficient funds!";
    return;
  }
...
function checkResult(results, bet) {
  if (results[0] === results[1] && results[1] === results[2]) {
      const win = bet * 10;
      points += win;
      resultText.textContent = `JACKPOT! +${win} points!`;
  } else if (
      results[0] === results[1] ||
      results[1] === results[2] ||
      results[0] === results[2]
      ...
      const win = bet * 3;
      points += win;
      resultText.textContent = `Two match! +${win} points!`;
  } else {
...
function updatePointsDisplay() {
  pointsDisplay.textContent = `Bálint coins: ${points}`;
}
